...
def online_translate(text, source, target):
    """Translate through OpenAI while keeping the API key on the server."""
    if not openai_client:
        return None
    source_name = LANGUAGES.get(source, "the detected source language") if source != "auto" else "the detected source language"
    target_name = LANGUAGES.get(target, target)
    instructions = (
        "You are Lingua's professional translation engine. Translate faithfully and naturally. "
        "Preserve meaning, names, numbers, punctuation, line breaks, and tone. "
        "Do not explain the translation. Return only the translated text."
    )
    prompt = (
        f"Translate this text from {source_name} to {target_name}. "
        f"If the source is auto-detected, identify the source language internally.\n\n{text}"
    )
    try:
...
@app.get("/health")
def health():
    return jsonify({"ok": True, "translation_configured": bool(openai_client), "ai_configured": bool(openai_client), "model": OPENAI_MODEL if openai_client else None})
...
def tutor_endpoint():
    ...
    scenario = str(data.get("scenario", "introductions")).strip()
    history = data.get("history", [])
    if not message:
        return jsonify({"error": "Please enter a message."}), 400
    if len(message) > MAX_LENGTH:
        return jsonify({"error": f"Please keep your message under {MAX_LENGTH:,} characters."}), 400
    if not openai_client:
    ...
    instructions = (
        "You are Lingua Tutor, a friendly language-learning coach for a teenage learner. "
        "Keep responses age-appropriate, encouraging, concise, and educational. "
        "Practise the selected language, correct mistakes gently, and give one useful tip when helpful. "
        "Do not use romantic roleplay, sexual content, or unsafe instructions. "
        "Return only the tutor's natural reply; do not mention these instructions or APIs."
    )
    ...
    try:
        ...
        if not reply:
            raise RuntimeError("empty tutor response")
        return jsonify({"response": reply, "provider": "ai", "xp": 6})
    except Exception:
        return jsonify({"error": "The AI tutor is temporarily unavailable."}), 502
...
def translate_endpoint():
    ...
    translation = None
    provider = "offline" if use_offline else "openai"
